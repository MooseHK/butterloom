...
/**
 * A quiet way back up one level, at the top of every page that is not the
 * collection. The wordmark goes home, but a wordmark is a logo before it is a
 * link — this says where back is in words.
 */
...
const css = `
  ...
  * { box-sizing: border-box; }
  ...
  h1 { margin: 0; font: 400 28px/1.15 ui-serif, Georgia, "Times New Roman", serif;
    text-wrap: balance; }
  .head { padding: 20px 0 18px; border-bottom: 1px solid var(--hairline); }
  .head .muted { margin-top: 7px; }
  .muted { margin: 0; color: var(--tertiary); font-size: 13px; }

  /* Uppercase system-ui at 10px is the label voice of the whole system: crumbs,
     panel headings, field labels and the placeholder all speak it. */
  .crumb { display: inline-flex; align-items: center; gap: 8px; min-height: 44px;
