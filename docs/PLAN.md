...
| 5 | ~~Return and exchange policy~~ | **Settled: refund rights differ by payment method.** COD orders get full change-of-mind refunds; bKash and card orders get exchange only. The regulatory floor applies to both — refunds within **10 days** of failed delivery via the same channel paid, and 48h-notify / 72h-refund for force majeure. Policy **must be published in Bengali**. Two sub-items still open: the change-of-mind **window in days**, and **who pays return shipping** — both pending the market research. |
